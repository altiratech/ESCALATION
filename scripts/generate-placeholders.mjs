...
const domains = ['economy', 'energy', 'unrest', 'military', 'cyber', 'diplomacy'];
const environments = ['generic', 'coastal', 'arctic', 'dense_city', 'industrial'];
const perspectives = ['satellite', 'street', 'news_frame', 'memo', 'ticker'];
...
const run = async () => {
  ...
  for (const domain of domains) {
    for (let severity = 0; severity <= 4; severity += 1) {
      const environment = environments[(index + severity) % environments.length];
      const perspective = perspectives[(index * 2 + severity) % perspectives.length];
